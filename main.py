...
# я их просто закинул в конфиг, поэтому на гите нет
...
commands = {"/start", "/help", "/cancel, /show_admins"}
    ...
    while True:
        ...
        if not any(task.get('deadline') for task in tasks.values()):
            ...
            for task_id, task in tasks.items():
                if 'assigned_to' in task and 'deadline' in task and task['deadline'] and task['status'] == 'Не сделано❌':
...
async def command_start(message: types.Message):
    ...
    if user_id in admin_ids:
        await message.answer("Привет, актив👋\nЯ <b>таскабот</b>, который поможет тебе получать, давать и выполнять задания к \
...
@dp.message_handler(commands=['show_admins'])
async def command_help(message: types.Message):
    admins_list = ["@" + reg_users[admin_id] for admin_id in admin_ids if admin_id in reg_users]
    text = "\n".join(admins_list) if admins_list else "Список администраторов пуст"
...
async def all_(message: types.Message):
    ...
    if mes == "я помню":
        await message.answer("пенис большой")
    if mes == "а члены дымятся":
        await message.answer("мужчины кончают ААААААААААА")
    if mes == "пи":
        await message.answer("скоро всем пи(ть)")
    if mes == "кб":
        await message.answer("круто(сосать)")
    if mes == "пм":
        await message.answer("мема не будет(")
    if mes == "инфа":
        await message.answer("Лытин пидор")
